...
contract Bazar{
    ...
    struct enchere {
     ...
     uint256 objet;
     address vendeur;
     uint typeEnchere; //1 pour classique, 2 pour hollandaise
     uint blockDeDepart;
   }
   ...
   function proposerALaVente(uint256 objet, uint typeEn, uint256 prixDepart) public{
        require(objMag.ownerOf(objet) == address(this));
        uint numero = numEnchere++;
        uint numBlock = block.number;
        if(typeEn == 1){
            Encheres[numero] = enchere(address(0),0,uint256(numBlock)+1000,objet,msg.sender, typeEn, numBlock);
        }
        else if(typeEn == 2){
            Encheres[numero] = enchere(address(0),prixDepart,uint256(numBlock)+1000,objet,msg.sender, typeEn, numBlock);
        }
    }
    
    function offre(uint indice) public payable{
        require(Encheres[indice].finEnchere > block.number);
        if(Encheres[indice].typeEnchere == 1){
        ...
        else if(Encheres[indice].typeEnchere == 2){
            require(Encheres[indice].meilleurAcheteur == address(0)); //Le premier arrivé, le premier servi
            uint numBlock = block.number;
            require(msg.value >= (Encheres[indice].meilleureOffre * (1 - (numBlock-Encheres[indice].blockDeDepart)/1000)));
            Encheres[indice].meilleureOffre = msg.value;
            Encheres[indice].meilleurAcheteur = msg.sender;
        }
    }
